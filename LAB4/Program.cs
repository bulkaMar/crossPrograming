...
class VersionCommand
{
    private void OnExecute()
    {
        Console.WriteLine("Author: Bulakh Marina group 33");
...
class RunCommand
{
    ...
    private void OnExecute()
    {
        ...
        if (Lab.ToLower() == "lab2")
        {
            runner.RunLab2(inputFilePath, outputFilePath, outputFilePath); // Assuming you want two output files for lab2
        }
    ...
    {
        // If the input file is specified, use that
        if (!string.IsNullOrEmpty(inputFile) && File.Exists(inputFile))
        {
            return inputFile;
        }

        // Check LAB_PATH environment variable
